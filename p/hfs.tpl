...
[box login]		
	<fieldset id='login'>
		<legend><img src="/~img27"> {.!账户.}</legend>
		<center>
		{.if| {.length|%user%.} |{:
            %user%
            {.if|{.can change pwd.} | 
                <button onclick='changePwd.call(this)' style='font-size:x-small;'>{.!更改密码.}</button>
            .}
            :}
            | <a href="~login">登录</a>
        .}
...
[box folder]
	<fieldset id='folder'>
		<legend><img src="/~img8"> {.!文件夹.}</legend>

       <div style='float:right; position:relative; top:-1em; font-weight:bold;'>
		{.if| {.length|{.?search.}.} | <a href="."><img src="/~img14"> {.!返回.}</a>
		| {.if| {.%folder% != / .} | <a href=".."><img src="/~img14"> {.!上一级.}</a> .}
		/if.}
		</div>

		<div id='breadcrumbs'>
		{.comment|we need this to start from 1 {.count|folder levels.}.}
		{.breadcrumbs|{:<a href="%bread-url%" {.if|{.length|%bread-name%.}|style='padding-left:{.calc|{.count|folder levels.}*0.7.}em;'.} /> {.if|{.length|%bread-name%.}|&raquo; %bread-name%|<img src="/~img1"> {.!根目录.}.}</a>:} .}
       </div>
        
		<div id='folder-stats'>%number-folders% {.!个文件夹.}, %number-files% {.!个文件.}, {.add bytes|%total-size%.}
		</div>
...
[box search]	
	...
	<fieldset id='search'>
		...
		<div style='margin-top:0.5em;' class='hidden popup'>
			<fieldset>
				<legend>{.!搜索位置.}</legend>
					<input type='radio' name='where' value='fromhere' checked='true' />  {.!此文件夹和子文件夹s.}
					<br><input type='radio' name='where' value='here' />  {.!只有这个文件夹.}
...
[box selection]	
	<fieldset id='select' class='onlyscript'>
    	...
    	<button onclick="
			...
			selectedChanged();
            ">{.!反选.}</button>
    	<button onclick='selectionMask.call(this)'>{.!筛选.}</button>
		<p style='display:none; margin-top:1em;'><span id='selected-number'>0</span> 个文件选中</p>
		</center>
	</fieldset>
...
[box upload]
	{.if| {.get|can upload.} |{:
		<fieldset id='upload'>
    		<legend><img src="/~img32"> {.!上传.}</legend>
    		...
    		<input type='submit' value='{.!上传.}' style='margin-top:0.5em;' />
    		</form>
		</fieldset>
	:}.}
...
[box actions]	
	<fieldset id='actions'>
		<legend><img src="/~img18"> {.!动作.}</legend>
		<center>
		{.if|{.can mkdir.}|
		...
		.}
		{.if|{.can comment.}|
		<button id='commentBtn' onclick='setComment.call(this)'>{.!评论.}</button>
		.}
		{.if|{.get|can delete.}|
		<button id='deleteBtn' onclick='if (confirm("{.!confirm.}")) submit({action:"delete"}, "{.get|url.}")'>{.!删除.}</button>

		{.if|{.and|{.!option.move.}|{.can move.}.}| <button id='moveBtn' onclick='moveClicked()'>{.!移动.}</button> .}
		.}
		{.if|{.can rename.}|
		<button id='renameBtn' onclick='
            var a = selectedItems();
                if (a.size() != 1)
				return alert("你必须选中一个文件");
			ezprompt(this.innerHTML, {type:"text"}, function(s){
				ajax("rename", {from:getItemName(a[0]), to:s});
		    });'>{.!重命名.}</button>
		.}
		{.if|{.get|can archive.}|
		<button id='archiveBtn' onclick='if (confirm("{.!confirm.}")) submit({}, "{.get|url|mode=archive|recursive.}")'>{.!存档.}</button>
		.}
		<a href="{.get|url|tpl=list|sort=|{.if not|{.length|{.?search.}.}|{:folders-filter=\|recursive:}.}.}">{.!获取列表.}</a>
		</center>
	</fieldset>

[box server info]
	<fieldset id='serverinfo'>
		<legend><img src="/~img0"> {.!服务信息.}</legend>
        <a href="https://www.lykns.tk/">返回主站</a>
		<br />
		<a>LYKNS Http File Server %version%</a>
		<br />{.!服务器时间.}: %timestamp%
		<br />{.!服务器开机时间.}: %uptime%
	</fieldset>
...
[+special:strings]
max s dl msg=此服务器上<b>同时</b>下载的用户数量有限制。<br>现在已达到此限制。 请稍后重试。
retry later=请稍后重试。
item folder=在文件夹中
no files=此文件夹中没有文件
no results=没有项目匹配您的搜索查询
confirm=你确定吗？ 
...
a { text-decoration:none; color:#0078d4; border:1px solid transparent; padding:0 0.1em; }
a:visited { color:#55F; }
...
{.if| {.length|%user%.} |{: {.append| %folder-resource%\hfs.comments.txt |{.filename|%item-resource%.}=已上传由 %user%
/append.} :}/if.}
...
$(function(){
    ...
    if  (x.size()) {
        ...
        $('#actions button:first').before(
            $("<button>{.!上传.}</button>").click(function(){ 
...
function moveClicked() {
    ezprompt("{.!输入目标文件夹.}", {type:'text'}, function(s){
...
function selectionMask() {
    ezprompt('{.!请输入文件掩码进行选择.}', {'type':'text', 'default':'*'}, function(s){
        if (!s) return false;
